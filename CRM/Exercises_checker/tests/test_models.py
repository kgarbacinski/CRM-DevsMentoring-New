...
class ExerciseStatusM2mTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.exercise_1 = Exercise.objects.create(
            name="Palindrome",
            language=cls.language,
            description="Write Palindrome",
            type="EASY",
        )
        cls.exercise_2 = Exercise.objects.create(
            name="Decorators",
            language=cls.language,
    ...
    def test_should_create_exercise_status_for_new_user_in_specified_language(self):
        exercise_status_1 = ExerciseStatus.objects.filter(
            exercise=self.exercise_1, user=self.user
        ).exists()
        exercise_status_2 = ExerciseStatus.objects.filter(
            exercise=self.exercise_2, user=self.user
        ).exists()
