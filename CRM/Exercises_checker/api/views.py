...
class ExerciseView(generics.ListAPIView):
    ...
    def get_all_exercises_quantity(self):
        ...
        all_exercises_quantity = (
            Exercise.objects.filter(language__user=user)
            .filter(language=language)
            .count()
        )
        done_exercises_quantity = (
            ExerciseStatus.objects.filter(exercise__language=language)
            .filter(user=user)
            .filter(done=True)
            .count()
        )
    ...
    def list(self, request, *args, **kwargs):
        (
            all_exercises_quantity,
            done_exercises_quantity,
        ) = self.get_all_exercises_quantity()
        (
            easy_exercises,
            easy_exercises_quantity,
            done_easy_exercises_quantity,
        ) = self.get_queryset_exercise_list(type="EASY")
        (
            medium_exercises,
            medium_exercises_quantity,
            done_medium_exercises_quantity,
        ) = self.get_queryset_exercise_list(type="MEDIUM")
        (
            hard_exercises,
            hard_exercises_quantity,
            done_hard_exercises_quantity,
        ) = self.get_queryset_exercise_list(type="HARD")
